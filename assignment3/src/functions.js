...
function parseGit(logArray) {
  ...
  var results = new Array(); //This array will contain a list of GitLog objects
  var hash, date, message;

  for(var k = 0; k < logArray.length; k++) {
	  
	hash = logArray[k].slice(0, logArray[k].indexOf(' ')); //get hash value
	date = logArray[k].slice(logArray[k].indexOf(' ') + 1, logArray[k].indexOf('\"')); //get date
	message = logArray[k].slice(logArray[k].indexOf('\"') + 1, logArray[k].length-1); //get message
	
	results[k] = new GitLog(hash, new Date(date), message);
  }
